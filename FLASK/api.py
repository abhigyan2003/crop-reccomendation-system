...
def predict():
    ...
    try:
        ...
        return jsonify({"error": f"Model files not found: {e}"})

    try:
        # Check if the request contains a file (for bulk prediction) or text input
        if "file" in request.files:
            logging.info("File detected in request")
            ...
            logging.info("Text detected in request")
            ...
            logging.warning("No valid input found in request")
            return jsonify({"error": "No valid input found in request"})
